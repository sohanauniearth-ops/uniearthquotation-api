    ...
    try:
        with zipfile.ZipFile(excel_path, 'r') as z:
            ...
            with z.open('xl/drawings/_rels/drawing1.xml.rels') as f:
                tree = etree.parse(f)
            rid_to_file = {r.get('Id'): r.get('Target').replace('../media/','') for r in tree.getroot()}
            ...
            for anchor in (root.findall('.//xdr:twoCellAnchor',ns) +
                ...
                if img_file and row not in row_to_img:
...
def generate():
    ...
    try:
        excel_bytes = build_excel(
            data.get('items',[]),data.get('client','Client'),
            data.get('project','Project'),data.get('qno','Q-001'),
            data.get('validity',10),data.get('today',''),data.get('is_sitc',False))
    ...
    for i,item in enumerate(items):
        ...
        if img_data:
            try:
                ...
                xl_img=XLImg(path)
                xl_img.anchor=AbsoluteAnchor(pos=XDRPoint2D(left_emu,top_emu),ext=XDRPositiveSize2D(cm_to_EMU(2.2),cm_to_EMU(1.6)))
                ws.add_image(xl_img)
            except Exception as e:
